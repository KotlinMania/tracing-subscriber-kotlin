...
kotlin {
    ...
    iosSimulatorArm64 {
        ...
        addToXcf(static = true)
        binaries.all {
            freeCompilerArgs += listOf("-Xoverride-konan-properties=minVersion.ios_simulator_arm64=12.0")
        }
    }
    tvosArm64 {
        configureBenchmarkCompilation()
        ...
        if (spmDir != null && spmDir.exists()) {
            spmDir.walkTopDown().forEach { file ->
                if (file.name == "Package.swift") {
                    ...
                    if (text.contains("_ExportedKotlinPackages_kotlinx_datetime_format_DateTimeFormatBuilder")) {
                        text =
                            text.replace(
                                Regex("""any\s+([A-Za-z0-9_.]+_ExportedKotlinPackages_kotlinx_datetime_format_DateTimeFormatBuilder_\w+)"""),
                                "Any",
                            )
                        modified = true
                    }
