...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `TracingSubscriber.swiftmodule/`
...
//      `import TracingSubscriber` above would fail at compile time.
//
//   2. The static archive `libTracingSubscriber.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "TracingSubscriber" }` and
//      `flattenPackage = "io.github.kotlinmania.tracingsubscriber"` configuration in
...
//      via the `TracingSubscriberLibrary` product.
//
...
final class TracingSubscriberExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "TracingSubscriber swift module imported cleanly")
    }
}
