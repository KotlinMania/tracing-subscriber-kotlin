# Claude Code Project Instructions — tracing-subscriber-kotlin

## Project Overview

This is **tracing-subscriber-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`tracing-subscriber`](https://crates.io/crates/tracing-subscriber).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/tracing-subscriber/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/tracing-subscriber>
